...
type ReceiveClientMsgHandle func(client *link.Session, msg packet.RAW) bool
type FrontendLinkCloseHandle func()
...
type frontendLink struct {
	...
	closeFlag              int32
	linkId                 uint64
	receiveClientMsgHandle ReceiveClientMsgHandle
	closeHandle            FrontendLinkCloseHandle
}
...
func (flink *frontendLink) LinkId() uint64 {
	return flink.linkId
}
...
func (flink *frontendLink) Close() {
	if atomic.CompareAndSwapInt32(&flink.closeFlag, 0, 1) {
		...
		if flink.closeHandle != nil {
			flink.closeHandle()
			flink.closeHandle = nil
		}

		flink.receiveClientMsgHandle = nil
	}
}
...
func (flink *frontendLink) SetCloseHandle(closeHandle FrontendLinkCloseHandle) {
	flink.closeHandle = closeHandle
}

func (flink *frontendLink) SetReceiveClientMsgHandle(receiveClientMsgHandle ReceiveClientMsgHandle) {
	flink.receiveClientMsgHandle = receiveClientMsgHandle
}
...
func (flink *frontendLink) newClient(waitId, clientId uint64) {
	...
	go func() {
		...
		for {
			...
			if flink.receiveClientMsgHandle != nil {
				needSendToBackend = flink.receiveClientMsgHandle(session, inMsg)
			}
